...
function getPolynom(...args) {
  return (x) => [...args].reverse().reduce((res, el, i) => res + el * (x ** i));
}
...
function logger(func, logFunc) {
  return (...args) => {
    const starts = `${func.name}(${JSON.stringify(args).slice(1, -1)}) starts`;
    logFunc(starts);
    const res = func(...args);
    const ends = `${func.name}(${JSON.stringify(args).slice(1, -1)}) ends`;
    logFunc(ends);
    return res;
  };
}
